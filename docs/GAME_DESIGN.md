...
9. During Evac Objective phase, UI shows `Proceed` on the first line and `to the evac zone` on the second, nearest-base radar points to the evac zone, and a flickering `3x3` world-unit green evac rectangle marks the target.
...
11. When the player fully enters the evac zone (player center at least player-radius inside zone edges), gameplay enters **Victory phase**: player is removed from the maze (hyperspace) and UI shows `Congratulations` until the player returns to the main menu.
12. For the first `3.0s` of Victory phase, dismiss input is ignored; after that, Victory phase ends when the player presses any input (same input-clear/press gating as Game Over), then the game transitions back to main menu.
...
- Failed spawn attempts (no spawn candidate when trying to start a spawn-prep sequence, including `no valid spawn heading`, `spawn point occupied`, or `6`-unit path gate blocked) reset that base's generation timer and wait one full generation interval before retry.
...
- If all damage-filtered headings fail spawn safety checks, spawn attempt fails for that interval (there is no fallback that ignores damaged-side direction blocking).
- If damaged-side filtering leaves no valid spawn heading for that attempt, spawn selection retries without this filter (same wall/clearance/occupancy gates still apply), so damaged bases can continue spawning.
...
- Each base also has an enemy generation timer initialized from that interval; timer counts down before spawn-prep can start.
- Spawn-prep sequence (one-shot, non-looping): when base/global caps allow spawn, timer is `<= 0`, and no alive enemy from that base remains inside the base footprint, the base captures a valid spawn candidate and starts a `2.0s` core-growth prep.
- When prep reaches `0`, enemy spawn is committed immediately from the captured candidate, the core returns to default size, spawn audio/event fires via normal `EnemySpawned` event, and generation timer restarts at `max(0, generationInterval - 2.0s)` so prep time is subtracted from post-spawn cooldown.
...
- Turret aim / fire direction: keyboard `Space` (and gamepad south button) fire along the **turret heading**.
  ...
  - **Mouse control** (menu option, default off): when enabled, the mouse cursor sets a **target** turret heading (direction from the player to the cursor, computed in world space via the gameplay camera). The turret does **not** snap to it — it slews toward the target along the shortest direction (CW/CCW) at a fixed angular speed `kMouseTurretTurnSpeedDegreesPerSecond` (`135 deg/s`), so a far cursor jump takes time to track. Rotating the **hull** still moves the turret instantly (the turret tracks hull turns), then the mouse slew continues toward the cursor. Holding the **primary mouse button** fires repeatedly (on cooldown) toward the current turret heading; `Space` still fires too. The mouse target is computed in `GameApp` and passed to `PlayerSystem` (`FrameInput::turretAbsoluteAimActive` / `turretAbsoluteAimHeading`), with the held mouse button as `FrameInput::fireHeld`.
...
- Start mode: player enters a fuel-ramp phase where fuel fills on HUD (and in simulation): from `0` to max for new game and respawn (`StartModeReason::NewGame` / `Respawn`) over `1.6s` (`kStartModeDurationSeconds`), or linearly from the pre-hit level to max when an enemy base is destroyed (`StartModeReason::BaseRefuel`) with duration scaled by missing fuel amount (`kStartModeDurationSeconds * (missingFuel / kFuelMax)`). New-game/respawn keep the start lock (hull/turret rotation enabled; movement/fire disabled). Base-refuel does **not** lock player controls (movement/fire remain enabled). There is no extra fuelling progress bar in the maze viewport; base-refuel is shown via the existing HUD fuel bar only. While start mode is active, the HUD fuel bar refresh uses the per-frame path (same as other start fuelling) instead of the slower snapshot interval.
- Fuel drain runs every fixed-step when gameplay allows fuel rules and player controls are not lock-disabled. Drain rate is `kFuelDrainPercentOfMaxPerSecond% * kFuelMax * (1 + speed / kPlayerFullVelocity) * mazeDensityDrainScale` per second (`speed` = current player velocity magnitude in world-units/second), integrated by fixed-step `dt`. `mazeDensityDrainScale = max(0, 1 - kFuelDrainReductionPerAdditionalMazeDensity * (clamp(menu maze density, 1..5) - 1))`, so each density step above 1 reduces drain by `kFuelDrainReductionPerAdditionalMazeDensity` (15%).
...
- On successful player core-hit destruction of an enemy base, if fuel is below `kFuelMax`, a base-refuel start mode runs: fuel ramps from the current level to `kFuelMax` over a proportional duration (`kStartModeDurationSeconds * (missingFuel / kFuelMax)`), avoiding a velocity discontinuity from the empty-fuel speed cap lifting mid-motion. If fuel is already full, no base-refuel mode is applied.
...
- Victory phase (`GameplayPhase::Victory`): entered only after successful evac-zone entry; the world keeps simulating with player driving disabled and player removed from maze rendering, `Congratulations` stays visible until return to menu; for the first `3.0s`, dismiss input is ignored, then player input returns to menu (same input-clear/press gating as Game Over).
...
- Projectile lifetime uses `kProjectileLifetimeSeconds` (`1.5s`) only while a projectile is outside active simulation coverage; while inside the full-tier zone (Chebyshev cell radius around the viewport center cell, matching enemy LOD `R`), lifetime countdown is paused and the projectile persists until it hits walls/targets or leaves that zone. That radius is larger than the on-screen viewport in cells, so visible shells stay covered without a separate viewport test.
...
- Player vs base (hard collision): when the player enters an undestroyed base footprint (with entity-radius clearance), the nearest damaged-footprint side decides the outcome. Hitting intact armor (side health `> 0`), or a breached side outside its **healthy-length** central third (the corners flanking a breach), destroys the player. Crashing through a breached side (`0`) into the central third (the exposed core) instead **explodes the base**: it is destroyed with the same score and base-refuel rewards as a player projectile core hit, and the player survives.
...
- Modes: `Wander`, `Watch`, and `Defend`.
- There is no movement pursuit mode.
- `Defend` enters when drone sees an alive player within `kDroneDetectRangeUnits`, the drone is not currently colliding/overlapping another enemy (`centerDistance >= kEnemyMutualKillDistanceUnits`), and the drone has fully exited any undestroyed base footprint with extra clearance (`1.0` world-unit).
...
- Additional base-clearance deadlock guard: if a drone remains in Watch for `>=8s` while still inside undestroyed-base clearance (`1.0` world-unit) and cannot satisfy normal Watch-exit checks, it forces `Wander` with an 8-way heading away from the nearest alive base center.
...
- Spawn heading lock: after spawn, all enemy types keep initial heading while inside their origin base footprint plus `1.0` world-unit clearance; AI turn/stop overrides are disabled until this clearance is exited so spawn-leave remains uninterrupted.
...
- While the menu is visible, a background simulation runs behind menu controls:
  ...
  - **Menu-only proactive avoidance** (`MenuBackgroundSimulation`): while following the segment waypoint direction, each enemy blends a small lateral bias toward **its own right** (lane-style offset) when another enemy is **ahead in a forward cone**, beyond preferred center spacing, and on a **near-opposite heading** (symmetric “both drivers hug their side” pass). Separately, if another enemy’s center lies inside a short **forward corridor** along that path (within lookahead distance and lateral half-width), steering nudges toward the **easier side**: obstacle-relative left/right by default; when nearly centered, it picks the side with **more wall clearance ahead** and **fewer neighbors** near short forward-lateral probe points. The blended direction is **angle-clamped per frame** (radians/second cap) before integration; gameplay separation / uncouple passes remain the overlap safety net.
