...
- No full-screen menu panel or dimmed dialog plate: controls sit directly on the menu background. Quit confirmation has no rounded panel behind the text (buttons and message only). The level `GuiSliderBar` uses a **2** px `YELLOW` border (same packed color as menu text), **2** px padding between border and track fill, **75%** of the menu gauge width (centered), and a focus ring with **2** px clear space outside the slider (`DrawFocusRing` expand derived from raylib line geometry in `MenuScreen.cpp`). `Density`, `Debug info`, `Start`, and `Quit` label text uses the same yellow (raylib `YELLOW` / matching packed GUI text color). `Start` and `Quit` have no button border or fill until keyboard focus (focus ring only); the quit confirmation dialog switches to bordered buttons for readability.
- Includes:
